...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  useEffect(() => {
    ...
    class App {
      ...
      initPasses() {
        ...
        if (!context || context.isContextLost?.()) {
          throw new Error('Hyperspeed WebGL context is not available.');
        }
    ...
    try {
      ...
      myApp.loadAssets()
        .then(() => {
          if (appRef.current !== myApp || myApp.disposed || !container.isConnected) return;
          myApp.init();
        })
        .catch(error => {
          console.warn('Hyperspeed failed to initialize.', error);
      ...
      console.warn('Hyperspeed WebGL is unavailable.', error);
      return undefined;
    }
