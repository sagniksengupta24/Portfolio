...
const useResponsiveDimension = (responsive, config, key) => {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      if (width <= 480 && config[`mobile${propName}`]) nextValue = config[`mobile${propName}`];
      else if (width <= 768 && config[`tablet${propName}`]) nextValue = config[`tablet${propName}`];
      else if (width <= 1024 && config[`desktop${propName}`]) nextValue = config[`desktop${propName}`];
